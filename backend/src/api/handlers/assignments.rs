...
use crate::api::requests::CreateAssignmentRequest;
use crate::api::models::AssignmentResponse;
...
pub async fn get_task_assignments(
    ...
    let assignments = state.assignment_service.get_by_task(task_id)
        .await?;
...
pub async fn get_user_assignments(
    ...
    let assignments = state.assignment_service.get_by_user(auth.user_id)
        .await?;
    Ok(Json(assignments.into_iter().map(Into::into).collect()))
}
...
pub async fn create_assignment(
    ...
    let task = state.task_service.get_by_id(&auth, task_id)
        .await?;
    ensure_can_manage_members(&auth, task.project_id, &state)
        .await?;

    let user_id = parse_id(&req.user_id)?;

    if req.role == AssignRole::Owner {
        let owner_user = state.user_service.get_by_id(user_id)
            .await?;
        ...
        let existing_assignments = state.assignment_service.get_by_task(task_id)
            .await?;
        for assignment in existing_assignments {
            if assignment.role == AssignRole::Owner {
                state.assignment_service.delete(assignment.id)
                    .await?;
            }
        }
    }
    ...
    let created = state.assignment_service.create(assignment)
        .await?;
...
pub async fn delete_assignment(
    ...
    let assignment = state.assignment_service.get_by_id(assignment_id)
        .await?;
    let task = state.task_service.get_by_id(&auth, assignment.task_id)
        .await?;
    ensure_can_manage_members(&auth, task.project_id, &state)
        .await?;

    state.assignment_service.delete(assignment_id)
        .await?;
